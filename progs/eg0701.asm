	TITLE	D:\MASM\eg0701\eg0701.cpp
	.386P
include listing.inc
if @Version gt 510
.model FLAT
...
; 12   :   long temp;	// 定义局部变量，用于返回值
; 13   :   __asm {	// 嵌入式汇编代码部分（参考例程4-18）
; 14   : 	mov ebx,d	;EBX＝数组地址

	mov	ebx, DWORD PTR _d$[ebp]

; 15   : 	mov ecx,num	;ECX＝数据个数

	mov	ecx, DWORD PTR _num$[ebp]

; 16   : 	xor eax,eax	;EAX保存和值

	xor	eax, eax

; 17   : 	xor edx,edx	;EDX＝指向数组元素

	xor	edx, edx
$mean1$1297:

; 18   : mean1:	add eax,[ebx+edx*4]	;求和

	add	eax, DWORD PTR [ebx+edx*4]

; 19   : 	add edx,1	;指向下一个数据
...
; 22   : 	cdq	;将累加和EAX符号扩展到EDX

	cdq

; 23   : 	idiv ecx	;有符号数除法，EAX＝平均值（余数在EDX中）
